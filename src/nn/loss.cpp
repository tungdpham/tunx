...
void Loss::compute_loss(const Tensor &predictions, const Tensor &targets, float &loss, stream s) {
...
void Loss::compute_gradient(const Tensor &predictions, const Tensor &targets, Tensor &gradient,
                            stream s) {
...
LossConfig CrossEntropyLoss::get_config() const {
  LossConfig config;
...
void CrossEntropyLoss::compute_loss_impl(const Tensor &predictions, const Tensor &targets,
                                         float &loss, stream s) {
...
void CrossEntropyLoss::compute_gradient_impl(const Tensor &predictions, const Tensor &targets,
                                             Tensor &gradient, stream s) {
  auto &device = predictions.device();
  size_t num_classes = predictions.shape().back();
  size_t batch_size = 1;
  for (size_t i = 0; i < predictions.dims() - 1; ++i) {
...
LossConfig MSELoss::get_config() const {
  LossConfig config;
  config.type = "mse";
  config.name = "MSELoss";
  return config;
...
void MSELoss::compute_loss_impl(const Tensor &predictions, const Tensor &targets, float &loss,
                                stream s) {
  if (predictions.device() != targets.device()) {
  ...
  if (predictions.device_type() == DeviceType::CPU) {
    return create_cpu_task(device, s, cpu::loss::compute_mse_loss, predictions.dtype(),
                           predictions.data_as(), targets.data_as(), loss, batch_size, output_size);
  }
#ifdef TUNX_USE_CUDA
  else if (predictions.device_type() == DeviceType::CUDA) {
...
void MSELoss::compute_gradient_impl(const Tensor &predictions, const Tensor &targets,
                                    Tensor &gradient, stream s) {
  if (predictions.device() != targets.device() || predictions.device() != gradient.device()) {
    throw std::runtime_error(
  ...
  if (predictions.device_type() == DeviceType::CPU) {
    return create_cpu_task(device, s, cpu::loss::compute_mse_gradient, predictions.dtype(),
                           predictions.data_as(), targets.data_as(), gradient.data_as(), batch_size,
                           output_size);
  }
#ifdef TUNX_USE_CUDA
  else if (predictions.device_type() == DeviceType::CUDA) {
...
LossConfig MAELoss::get_config() const {
  LossConfig config;
  config.type = "mae";
  config.name = "MAELoss";
  return config;
...
void MAELoss::compute_loss_impl(const Tensor &predictions, const Tensor &targets, float &loss,
                                stream s) {
  if (predictions.device() != targets.device()) {
...
void MAELoss::compute_gradient_impl(const Tensor &predictions, const Tensor &targets,
                                    Tensor &gradient, stream s) {
  if (predictions.device() != targets.device() || predictions.device() != gradient.device()) {
    throw std::runtime_error(
...
LossConfig HuberLoss::get_config() const {
  LossConfig config;
  config.type = "huber";
...
void HuberLoss::compute_loss_impl(const Tensor &predictions, const Tensor &targets, float &loss,
                                  stream s) {
  if (predictions.device() != targets.device()) {
...
void HuberLoss::compute_gradient_impl(const Tensor &predictions, const Tensor &targets,
                                      Tensor &gradient, stream s) {
  if (predictions.device() != targets.device() || predictions.device() != gradient.device()) {
    throw std::runtime_error(
...
std::unique_ptr<Loss> LossFactory::create(const std::string &loss_type) {
...
std::unique_ptr<Loss> LossFactory::create_from_config(const LossConfig &config) {
...
std::unique_ptr<Loss> LossFactory::create_cross_entropy(bool use_logits, double epsilon) {
  return std::make_unique<CrossEntropyLoss>(use_logits, epsilon);
}

std::unique_ptr<Loss> LossFactory::create_mse() { return std::make_unique<MSELoss>(); }

std::unique_ptr<Loss> LossFactory::create_mae() { return std::make_unique<MAELoss>(); }

std::unique_ptr<Loss> LossFactory::create_huber(double delta) {
  return std::make_unique<HuberLoss>(delta);
}
