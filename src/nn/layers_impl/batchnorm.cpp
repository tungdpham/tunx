...
Tensor BatchNormImpl::backward_impl(const Tensor &grad_output, Residuals &residuals) {
...
LayerConfig BatchNormImpl::get_config() const {
  LayerConfig config;
...
Vec<size_t> BatchNormImpl::compute_output_shape(const Vec<size_t> &input_shape) const {
  return input_shape;
}
