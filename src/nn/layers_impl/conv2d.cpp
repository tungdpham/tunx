...
Tensor Conv2DImpl::forward_impl(const Tensor &input, Residuals &residuals) {
  if (input.dims() != 4) {
  ...
  if (channels != in_channels_) {
    std::cerr << "Input shape: " << channels << " channels, expected: " << in_channels_
              << " channels" << std::endl;
    throw std::invalid_argument("Input channel size mismatch in Conv2DImpl");
  }
...
Tensor Conv2DImpl::backward_impl(const Tensor &grad_output, Residuals &residuals) {
  ...
  if (channels != out_channels_) {
    std::cerr << "Gradient shape: " << channels << " channels, expected: " << out_channels_
              << " channels" << std::endl;
    throw std::invalid_argument("Gradient channel size mismatch in Conv2DImpl");
  }
...
LayerConfig Conv2DImpl::get_config() const {
  LayerConfig config;
...
Vec<size_t> Conv2DImpl::compute_output_shape(const Vec<size_t> &input_shape) const {
  if (input_shape.size() != 4) {
    throw std::invalid_argument("Conv2DImpl expects 4D input including batch size");
  }
...
std::shared_ptr<Conv2DImpl> Conv2DImpl::create_from_config(const LayerConfig &config) {
