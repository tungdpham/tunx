...
void ConvTranspose2DOp::init(OpContext &ctx, const Config &config) {
  ...
  // For ConvTranspose2D using conv2d_dgrad, the "forward" pass computes the gradient wrt data
  // of a corresponding normal conv.
  // The corresponding normal conv goes from `out_channels` to `in_channels`.
  // So its weight shape is [in_channels, kernel_h, kernel_w, out_channels].
  Param weight = ctx.make_param({in_channels, kernel_h, kernel_w, out_channels});
...
Tensor ConvTranspose2DOp::forward(OpContext &ctx, const Tensor &input, const Param &weight,
                                  const Param &bias, const Config &config) {
  if (input.dims() != 4) {
    throw std::invalid_argument("ConvTranspose2D: Input tensor must be 4-dimensional (NHWC)");
  }
  ...
  if (channels != config.in_channels) {
    std::cerr << "Input shape: " << channels << " channels, expected: " << config.in_channels
              << " channels" << std::endl;
    throw std::invalid_argument("Input channel size mismatch in ConvTranspose2DOp");
  }

  if (config.use_bias) {
    throw std::invalid_argument(
        "ConvTranspose2D: bias is not supported yet because conv2d_dgrad does not support bias "
        "addition.");
  }
  ...
  size_t output_h = (input_h - 1) * config.stride_h - 2 * config.pad_h + config.kernel_h;
  size_t output_w = (input_w - 1) * config.stride_w - 2 * config.pad_w + config.kernel_w;
  ...
  ctx.engine->conv2d_dgrad(ctx.handle, stats, input.data_as<void>(), weight.data_as<void>(),
                           output.data_as<void>(), ws.data_as<void>(), type_desc);

  return output;
}

Tensor ConvTranspose2DOp::backward(OpContext &ctx, const Tensor &grad_output, Param &weight,
                                   Param &bias, const Config &config) {
  if (grad_output.dims() != 4) {
    throw std::invalid_argument("ConvTranspose2D: Gradient tensor must be 4-dimensional (NHWC)");
  }
  ...
  if (channels != config.out_channels) {
    std::cerr << "Gradient shape: " << channels << " channels, expected: " << config.out_channels
              << " channels" << std::endl;
    throw std::invalid_argument("Gradient channel size mismatch in ConvTranspose2DOp");
  }
  ...
  size_t output_h = grad_output.dim(1);
  size_t output_w = grad_output.dim(2);
  ...
  size_t max_ws = std::max(ws_req.fwd_workspace, ws_req.bwd_workspace);
  Tensor ws = ctx.make_tensor({max_ws}, DType_t::BYTE);

  ctx.engine->conv2d_wgrad(ctx.handle, stats, input.data_as<void>(), grad_output.data_as<void>(),
                           weight.grad_as<void>(), ws.data_as<void>(), type_desc);

  ctx.engine->conv2d_fwd(ctx.handle, stats, grad_output.data_as<void>(), weight.data_as<void>(),
                         nullptr, grad_input.data_as<void>(), ws.data_as<void>(), type_desc);

  return grad_input;
}

LayerConfig ConvTranspose2DOp::get_config(const Config &config, const std::string &name) {
  LayerConfig lcfg;
  lcfg.name = name;
  lcfg.type = TYPE_NAME;
...
ConvTranspose2DOp::Config ConvTranspose2DOp::parse_config(const LayerConfig &config) {
  Config c;
  ...
  c.use_bias = config.get<bool>("use_bias", false);
  return c;
}

Vec<Vec<size_t>> ConvTranspose2DOp::output_shapes(const Vec<Vec<size_t>> &input_shapes,
                                                  const Config &config) {
  if (input_shapes.empty() || input_shapes[0].size() != 4) {
    throw std::invalid_argument("ConvTranspose2DOp expects 4D input including batch size");
  }

  size_t batch_size = input_shapes[0][0];
  size_t output_h = (input_shapes[0][1] - 1) * config.stride_h - 2 * config.pad_h + config.kernel_h;
  size_t output_w = (input_shapes[0][2] - 1) * config.stride_w - 2 * config.pad_w + config.kernel_w;

  return {{batch_size, output_h, output_w, config.out_channels}};
}

ConvTranspose2D::ConvTranspose2D(size_t in_channels, size_t out_channels, size_t kernel_h,
                                 size_t kernel_w, size_t stride_h, size_t stride_w, size_t pad_h,
                                 size_t pad_w, bool use_bias, const std::string &name)
    : FunctionalLayer(ConvTranspose2DOp::Config{in_channels, out_channels, kernel_h, kernel_w,
