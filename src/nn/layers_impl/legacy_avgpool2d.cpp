...
LegacyAvgPool2DImpl::LegacyAvgPool2DImpl(size_t pool_h, size_t pool_w, size_t stride_h,
...
Tensor LegacyAvgPool2DImpl::forward_impl(const Tensor &input, Residuals &residuals) {
  if (input.dims() != 4) {
  ...
  engine_->legacy_avgpool2d_fwd(engine_handle_, input.data_as<void>(), output.data_as<void>(),
...
Tensor LegacyAvgPool2DImpl::backward_impl(const Tensor &grad_output, Residuals &residuals) {
  ...
  engine_->legacy_avgpool2d_bwd(engine_handle_, grad_output.data_as<void>(),
...
LayerConfig LegacyAvgPool2DImpl::get_config() const {
  LayerConfig config;
...
Vec<size_t> LegacyAvgPool2DImpl::compute_output_shape(const Vec<size_t> &input_shape) const {
  if (input_shape.size() != 4) {
    throw std::invalid_argument("LegacyAvgPool2DImpl expects 4D input including batch size");
  }
...
std::shared_ptr<LegacyAvgPool2DImpl> LegacyAvgPool2DImpl::create_from_config(
  ...
  return std::make_shared<LegacyAvgPool2DImpl>(pool_h, pool_w, stride_h, stride_w, pad_h, pad_w,
                                               config.name);
}
