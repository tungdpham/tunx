 ...
 */
#include "nn/layers_impl/legacy_avgpool2d.hpp"
...
Vec<Vec<size_t>> LegacyAvgPool2DOp::output_shapes(const Vec<Vec<size_t>> &input_shapes,
                                                  const Config &config) {
  if (input_shapes.size() != 1 || input_shapes[0].size() != 4) {
    throw std::invalid_argument("LegacyAvgPool2DOp expects 4D input including batch size");
  }
  const auto &shape = input_shapes[0];
  size_t batch_size = shape[0];
  size_t channels = shape[1];
  size_t padded_h = shape[2] + 2 * config.pad_h;
  size_t padded_w = shape[3] + 2 * config.pad_w;
...
Tensor LegacyAvgPool2DOp::forward(OpContext &ctx, const Tensor &input, const Config &config) {
  ...
  ctx.engine->legacy_avgpool2d_fwd(ctx.handle, input.data_as<void>(), output.data_as<void>(),
                                   batch_size, channels, input_h, input_w, output_h, output_w,
                                   config.pool_h, config.pool_w, config.stride_h, config.stride_w,
                                   config.pad_h, config.pad_w, type_desc);

  return output;
}

Tensor LegacyAvgPool2DOp::backward(OpContext &ctx, const Tensor &grad_output,
                                   const Config &config) {
  ...
  size_t input_h = (grad_shape[2] - 1) * config.stride_h + config.pool_h - 2 * config.pad_h;
  size_t input_w = (grad_shape[3] - 1) * config.stride_w + config.pool_w - 2 * config.pad_w;
  ...
  ctx.engine->legacy_avgpool2d_bwd(
      ctx.handle, grad_output.data_as<void>(), grad_input.data_as<void>(), batch_size, channels,
      input_h, input_w, output_h, output_w, config.pool_h, config.pool_w, config.stride_h,
      config.stride_w, config.pad_h, config.pad_w, type_desc);

  return grad_input;
}

LayerConfig LegacyAvgPool2DOp::get_config(const Config &config, const std::string &name) {
  LayerConfig lcfg;
  lcfg.name = name;
  lcfg.type = TYPE_NAME;
...
LegacyAvgPool2DOp::Config LegacyAvgPool2DOp::parse_config(const LayerConfig &config) {
  Config c;
