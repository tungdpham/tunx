...
Tensor MaxPool2DImpl::forward_impl(const Tensor &input, Residuals &residuals) {
  const auto &shape = input.shape();
  if (shape.size() != 4) {
    throw std::runtime_error("MaxPool2DImpl: input must be 4D (NHWC format)");
  }
...
Tensor MaxPool2DImpl::backward_impl(const Tensor &grad_output, Residuals &residuals) {
  const Tensor &mask_indices = residuals["mask_indices"];

  const auto &grad_shape = grad_output.shape();
  if (grad_shape.size() != 4) {
    throw std::runtime_error("MaxPool2DImpl: grad_output must be 4D (NHWC format)");
  }
...
LayerConfig MaxPool2DImpl::get_config() const {
  LayerConfig config;
...
Vec<size_t> MaxPool2DImpl::compute_output_shape(const Vec<size_t> &input_shape) const {
  if (input_shape.size() != 4) {
    throw std::invalid_argument("MaxPool2DImpl: input shape must be 4D (NHWC format)");
  }
