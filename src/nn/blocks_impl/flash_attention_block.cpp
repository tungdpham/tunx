...
Tensor FlashAttentionBlockOp::forward(OpContext &ctx, const Tensor &input, Layer q_proj,
  ...
  Tensor k_heads = ctx.make_tensor({batch_size, config.num_heads, seq_len, head_dim}, ctx.io_dtype);
  Tensor v_heads = ctx.make_tensor({batch_size, config.num_heads, seq_len, head_dim}, ctx.io_dtype);
...
Tensor FlashAttentionBlockOp::backward(OpContext &ctx, const Tensor &grad_output, Layer q_proj,
                                       Layer k_proj, Layer v_proj, Layer out_proj,
                                       const Config &config) {
...

  {
    ...
    ctx.engine->transpose(ctx.handle, t_stats, v.data_as<void>(), v_heads.data_as<void>(), nullptr,
                          t_desc);
    ctx.engine->transpose(ctx.handle, t_stats, attn_out.data_as<void>(), attn_heads.data_as<void>(),
                          nullptr, t_desc);
  }
...
FlashAttentionBlock::FlashAttentionBlock(size_t embed_dim, size_t num_heads, bool is_causal,
    ...
    : FunctionalLayer<FlashAttentionBlockOp>(
          FlashAttentionBlockOp::Config{embed_dim, num_heads, is_causal}, name) {
...
Layer FlashAttentionBlock::create_from_config(const LayerConfig &config) {
  ...
  return FlashAttentionBlock(cfg.embed_dim, cfg.num_heads, cfg.is_causal,
                             config.name.empty() ? TYPE_NAME : config.name);
}
