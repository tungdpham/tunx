...
TEST_F(DELAllocatorV2Test, AllocateBasic) {
  ...
  EXPECT_EQ(allocator_->in_use(), aligned_size);
...
TEST_F(DELAllocatorV2Test, ReclaimAndCoalesce) {
  ...
  
  {
    ...
    EXPECT_EQ(allocator_->in_use(), size1 + size2 + size3);
  }
  // All ptrs are out of scope, they should be reclaimed.
  // Because they are all the active allocations, the empty slab should be merged/cleared partially based on logic,
  // but in_use() must definitely be 0.
  EXPECT_EQ(allocator_->in_use(), 0);
}
...
TEST_F(DELAllocatorV2Test, AllocationHooks) {
  ...
  auto hook = [&last_allocated](size_t allocated) {
