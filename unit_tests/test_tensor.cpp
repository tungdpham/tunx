...
protected:
  void SetUp() override {
    small_tensor = Tensor({1, 1, 2, 2}, DType_t::FP32);
    fill(small_tensor, 1.0f);

    large_tensor = Tensor({2, 3, 4, 4}, DType_t::FP32);
    fill(large_tensor, 2.0f);
  }
...
TEST_F(TensorTest, Constructor4D) {
  Tensor tensor = Tensor({2, 3, 4, 4}, DType_t::FP32);
...
TEST_F(TensorTest, ConstructorWithShape) {
  ...
  Tensor tensor = Tensor(shape, DType_t::FP32);
...
TEST_F(TensorTest, TensorAddition) {
  Tensor tensor1 = Tensor({1, 1, 2, 2}, DType_t::FP32);
  Tensor tensor2 = Tensor({1, 1, 2, 2}, DType_t::FP32);
...
TEST_F(TensorTest, SameShapeComparison) {
  Tensor tensor1 = Tensor({2, 3, 4, 5}, DType_t::FP32);
  Tensor tensor2 = Tensor({2, 3, 4, 5}, DType_t::FP32);
  Tensor tensor3 = Tensor({2, 3, 4, 6}, DType_t::FP32);
...
TEST_F(TensorTest, AdditionShapeMismatch) {
  Tensor tensor1 = Tensor({1, 1, 2, 2}, DType_t::FP32);
  Tensor tensor2 = Tensor({1, 1, 3, 3}, DType_t::FP32);
...
TEST_F(TensorTest, MultiBatchAccess) {
  Tensor tensor = Tensor({2, 1, 2, 2}, DType_t::FP32);
...
TEST_F(TensorTest, MultiChannelAccess) {
  Tensor tensor = Tensor({1, 3, 2, 2}, DType_t::FP32);
...
TEST_P(TensorSizeTest, ConstructorAndSize) {
  ...
  Tensor tensor = Tensor({batch, channels, height, width}, DType_t::FP32);
