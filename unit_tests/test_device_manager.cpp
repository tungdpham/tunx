...
TEST_F(DeviceManagerTest, DeviceManagerUtilities) {
  ...
  EXPECT_FALSE(manager.hasDevice("CUDA:9999"));
}
...
TEST_F(DeviceManagerTest, ErrorConditions) {
  ...
  EXPECT_THROW(manager.getDevice("CUDA:9999"), std::runtime_error);

  EXPECT_NO_THROW(manager.removeDevice("CUDA:9999"));
}
...
TEST_F(DeviceManagerTest, CUDADeviceDiscovery) {
  ...
  if (found_gpu) {
    std::cout << "CUDA CUDA device found and tested successfully" << std::endl;
  } else {
    std::cout << "No CUDA CUDA devices found (this is not an error)" << std::endl;
  }
}
#endif
