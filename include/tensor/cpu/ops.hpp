...
template <typename T>
void im2col_pad_1_stride_1_kernel_3(const T *input_data, T *col_data, size_t batch_size,
                                    size_t channels, size_t height, size_t width, size_t output_h,
                                    size_t output_w) {
...
inline void im2col(DType_t dtype, const void *input_data_ptr, void *col_data_ptr, size_t batch_size,
  ...
  DISPATCH_ANY_DTYPE(dtype, T, {
    const T *input_data = static_cast<const T *>(input_data_ptr);
...
inline void col2im(DType_t dtype, const void *col_data_ptr, void *result_data_ptr,
                   size_t batch_size, size_t channels, size_t height, size_t width, size_t kernel_h,
  ...
  DISPATCH_ANY_DTYPE(dtype, T, {
    ...
    T *result_data = static_cast<T *>(result_data_ptr);
    size_t col_width = output_h * output_w;
            ...
            for (size_t kw = 0; kw < kernel_w; ++kw) {
              ...
              for (size_t h = h_start; h < h_end; ++h) {
                ...
                if (stride_w == 1) {
                  ...
                  kernel::cpu::add(dtype, result_row + w_out_start, col_row_ptr,
                                   result_row + w_out_start, valid_width);
...
inline void pad(DType_t dtype, const void *input_data_ptr, void *result_data_ptr, size_t batch_size,
  ...
  DISPATCH_ANY_DTYPE(dtype, T, {
    ...
    parallel_for_2d(batch_size, channels, [&](size_t n, size_t c) {
      ...
      for (size_t h = 0; h < height; ++h) {
        ...
        // copy the row over
        std::copy(
            &input_data[((n * channels + c) * height + h) * width],
        ...
        // right side
        std::fill(&result_data[((n * channels + c) * padded_height + new_h) * padded_width + pad_w +
                               width],
                  &result_data[((n * channels + c) * padded_height + new_h) * padded_width +
                               padded_width],
                  t_value);
      }
...
inline void unpad(DType_t dtype, const void *input_data_ptr, void *result_data_ptr,
                  size_t batch_size, size_t channels, size_t height, size_t width, size_t pad_h,
                  size_t pad_w) {
  DISPATCH_ANY_DTYPE(dtype, T, {
    const T *input_data = static_cast<const T *>(input_data_ptr);
    T *result_data = static_cast<T *>(result_data_ptr);
    ...
    parallel_for_2d(batch_size, channels, [&](size_t n, size_t c) {
      for (size_t h = 0; h < height; ++h) {
        ...
        std::copy(&input_data[((n * channels + c) * padded_height + src_h) * padded_width + pad_w],
...
inline void crop(DType_t dtype, const void *input_data_ptr, void *result_data_ptr,
                 size_t batch_size, size_t channels, size_t height, size_t width, size_t start_h,
...
inline void transpose_2d(DType_t dtype, const void *src_ptr, void *dst_ptr, size_t rows,
                         size_t cols) {
  DISPATCH_ANY_DTYPE(dtype, T, {
...
inline void nchw_to_cnhw(DType_t dtype, const void *src_ptr, void *dst_ptr, size_t batch_size,
...
inline void cnhw_to_nchw(DType_t dtype, const void *src_ptr, void *dst_ptr, size_t batch_size,
                         size_t channels, size_t height, size_t width) {
  DISPATCH_ANY_DTYPE(dtype, T, {
    const T *src = static_cast<const T *>(src_ptr);
    T *dst = static_cast<T *>(dst_ptr);
 ...
 */
template <typename T>
void slice_batch(const T *input_data, T *result_data, size_t start_batch, size_t end_batch,
                 size_t stride_0) {
 ...
 */
template <typename T>
void split(const T *input_data, Vec<T *> &results, size_t batch_size, size_t num_splits,
           size_t stride_0) {
  ...
  for (size_t i = 0; i < num_splits; ++i) {
    ...
    slice_batch<T>(input_data, results[i], start, end, stride_0);
  }
}
