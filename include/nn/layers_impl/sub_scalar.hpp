...
struct SubScalarOp {
  static constexpr const char *TYPE_NAME = "sub_scalar";
  ...
  static Config parse_config(const LayerConfig &config) { return Config{config.get<double>("scalar", 0.0)}; }
  static Vec<Vec<size_t>> output_shapes(const Vec<Vec<size_t>> &input_shapes, const Config &config);
};

using SubScalar = FunctionalLayer<SubScalarOp>;
