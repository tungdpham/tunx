...
for run in {1..5}; do
    for version in {1..4}; do
        ...
        log="logs/torch_eager_v${version}_run${run}.log"
        memory_log="logs/torch_eager_v${version}_run${run}_vram.csv"
        ...
        if [ "$status" -ne 0 ]; then
            echo "Benchmark failed: V${version}, run ${run}" \
                | tee -a "$log" >&2
        fi
...
' > logs/torch_eager_nohup_master.log 2>&1 &
