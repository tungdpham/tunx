...
# ===== GLOO P2P FIX =====
def _p2p_device(device):
    return torch.device("cpu") if dist.get_backend() == "gloo" else device
...
def _empty_p2p_like(t):
    return torch.empty(t.shape, dtype=t.dtype, device=_p2p_device(t.device))
# ========================
def ensure_dir(path):
    p = Path(path)
...
def _make_model_configs(seq_len=512):
    ...
    return {
        "gpt2": {
            "stage0_cls": lambda: GPT2Stage0(_GPT2_NUM_LAYERS * 2 // 3),
            "stage1_cls": lambda: GPT2Stage1(_GPT2_NUM_LAYERS // 3),
...
def send_ctrl(device, cmd, batch_size):
    ctrl = torch.tensor([cmd, batch_size], dtype=torch.int64, device="cpu")
    dist.send(ctrl, dst=1)
def recv_ctrl(device):
    ctrl = torch.empty(2, dtype=torch.int64, device="cpu")
    dist.recv(ctrl, src=0) 
    return int(ctrl[0].item()), int(ctrl[1].item())
        ...
        if rank == 0:
            ...
            for epoch in range(cfg["epochs"]):
                ...
                for step, (x,y) in enumerate(train_loader):
                    ...
                    dist.send(_to_p2p(a), dst=1)
                    dist.send(_to_p2p(y), dst=1)
                    grad_a_recv = _empty_p2p_like(a)
                    dist.recv(grad_a_recv, src=1)
                    grad_a = grad_a_recv.to(device, non_blocking=True)
            ...
            while True:
                ...
                t0 = time.time()
                p2p_dev = _p2p_device(device)
                act = torch.empty((bsz, *cfg["act_tail"]), dtype=torch.float32, device=p2p_dev)
                if is_gpt2:
                    y = torch.empty((bsz, cfg["act_tail"][0]), dtype=torch.int64, device=p2p_dev)
                else:
                    y = torch.empty((bsz,), dtype=torch.int64, device=p2p_dev)
                ...
                if cmd == CMD_TRAIN:
                    ...
                    dist.send(_to_p2p(act.grad), dst=0)
