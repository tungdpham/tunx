...
int main(int argc, char** argv) {
    ...
    if (pt_dir.empty() || tunx_dir.empty()) {
        std::cerr << "Usage: " << argv[0] << " --model <name> --pt_dir <dir> --tunx_dir <dir>" << std::endl;
        return 1;
    }
    ...
    for (auto& edge : graph.edges()) {
        ...
        // BN also has running mean and var, but in TunX Layer::params() only returns trainable parameters (weight, bias).
        // For full correctness, running stats should be loaded too, but TunX's BatchNorm 
        // doesn't expose running_mean in params(). If it diverges, we might need to expose them.
        // For 1 step equivalence, initial running stats are 0 and 1, which both frameworks default to.
    }
    
    std::cout << "Loading inputs and labels..." << std::endl;
    bool is_lm = (model_name.find("gpt2") != std::string::npos);
    Vec<size_t> input_shape = {1, 224, 224, 3};
    Vec<size_t> label_shape = {1};
    if (is_lm) {
        input_shape = {1, 1024};
        label_shape = {1, 1024};
    }
